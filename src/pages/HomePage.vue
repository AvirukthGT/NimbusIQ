...
/* ---------- layout variables ---------- */
:root{
  --navH: 72px;      /* height of your absolute navbar */
  --footH: 60px;     /* height of your footer */
  /* panel fallbacks if not defined globally */
...
/* ---------- page shell (no flex here) ---------- */
.stage{
  position: relative;
  min-height: calc(100dvh - var(--footH));
  padding-top: var(--navH);      /* clear the absolute navbar */
  overflow: hidden;
}
...
/* ---------- hero ---------- */
.hero{ position:relative; z-index:3; text-align:center; padding:0 0 6px; }
.title{ font-size:3rem; margin:0; letter-spacing:.3px }
.subtitle{ color:var(--muted); margin:.6rem 0 1.2rem }

/* ---------- grid cards ---------- */
.grid{ display:grid; gap:22px; padding:0 0 24px; position:relative; z-index:3 }
...
.bg-grid::after{ content:""; position:absolute; inset:0;
  background:radial-gradient(1200px 300px at 50% -120px, rgba(37,194,246,.25), transparent 70%) }
...
.day{
  text-align:center; padding:12px; border-radius:14px; background:rgba(255,255,255,.05);
  border:1px solid var(--panel-border);
